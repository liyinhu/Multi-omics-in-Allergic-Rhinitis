...
Age_Sex_Effects_on_proteins=data.frame(matrix(ncol = 8,nrow = 362))
colnames(Age_Sex_Effects_on_proteins)=c("Age_Estimate","Age_SE","Age_T_value","Age_P_value","Sex_Estimate","Sex_SE","Sex_T_value","Sex_P_value")
rownames(Age_Sex_Effects_on_proteins)=names(data)[1:362]
...
Adjusted_data[,k]=Merge_data[,k]-Merge_data[,364]*as.numeric(Age_Sex_Effects_on_proteins[k,1])-Merge_data[,363]*as.numeric(Age_Sex_Effects_on_proteins[k,5])
}

#Export correction coefficients and adjusted results
write.csv(Adjusted_data,'Agesex_adjusted_protein_levels.csv',row.names = T)
write.csv(Age_Sex_Effects_on_proteins,'AgeSex_Effects.csv',row.names = T)
